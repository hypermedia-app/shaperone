...
function getProperties(env: ShaperoneEnvironment, constraint: GraphPointer): PropertyShape[] {
...
export function combineProperties(env: ShaperoneEnvironment, shape: Shape): { properties: PropertyShape[]; logicalConstraints: LogicalConstraints } {
  ...
  for (const constraint of shape.pointer.out(sh.and).toArray()) {
    const list = constraint.list()
    if (list) {
      properties = [...properties, ...getProperties(env, constraint)]
