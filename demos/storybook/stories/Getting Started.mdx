...
This sets the default appearance and behaviour of the form and must be called before the component is used in a page.

```javascript
import { configure } from 'shaperone'

await configure(({ renderer, components, editors, validation }) => {
  // renderer => Changes the form's appearance
  // components => Add or repalce editor implementations
  // editors => Modify the editors' metadata and their priorities
  // validation => Set up validation
})
```

Individual form instances can be customised by calling the `configure` method on the form instance or setting
the configuration function to the `configuration` property:

```ts
import type { ConfigCallback, ShaperoneForm } from 'shaperone'

const form: ShaperoneForm = document.querySelector('shaperone-form')

const formSetup: ConfigCallback = () => { /* ... */ }

form.configure(formSetup)
// or
form.configuration = formSetup
```

The latter method can be useful when used in using a templating library like [lit-html](https://lit.dev).
